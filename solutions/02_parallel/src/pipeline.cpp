...
struct WorkItem {
    ...
    financial::common::EncodedRecord encoded;
};
...
ParallelOutput process_records(
    ...
    ParallelConfig config)
{
    ...
    for (std::size_t worker = 0; worker < config.worker_count; ++worker) {
        workers.emplace_back([&] {
            while (auto item = queue.pop()) {
                auto record = process_one(item->encoded, reference);
                output_bytes.fetch_add(encoded_size(record), std::memory_order_relaxed);
                records[item->output_index] = std::move(record);
            }
        });
    }
    ...
    for (std::size_t i = 0; i < input.size(); ++i) {
        ...
        queue.push(WorkItem{.output_index = i, .encoded = input[i]});
    }
    queue.close();
    workers.clear();

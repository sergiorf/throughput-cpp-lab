...
void record_deallocation_unknown_size() noexcept
{
    deallocation_count.fetch_add(1, std::memory_order_relaxed);
}
...
void* operator new(std::size_t size)
{
    ...
    throughput::record_allocation(size);
    return ptr;
}
...
{
    throughput::record_deallocation_unknown_size();
    std::free(ptr);
}
...
{
    throughput::record_deallocation(size);
    std::free(ptr);
}
